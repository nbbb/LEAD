...
class PositionAttention(nn.Module):
    def __init__(self, max_length, in_channels=512, num_channels=64, l=16, mode='nearest', **kwargs):
        super().__init__()
        self.max_length = max_length
        self.k_encoder = nn.Sequential(
            encoder_layer(in_channels, num_channels, s=2),
            encoder_layer(num_channels, num_channels, s=2),
            encoder_layer(num_channels, num_channels, s=2),
            encoder_layer(num_channels, int(num_channels), s=2)
        )
        self.k_decoder = nn.Sequential(
            decoder_layer(num_channels, num_channels, scale_factor=2, mode=mode),
            decoder_layer(num_channels, num_channels, scale_factor=2, mode=mode),
            decoder_layer(num_channels, num_channels, scale_factor=2, mode=mode),
            decoder_layer(num_channels, in_channels, size=l, mode=mode)
        )
    ...
    def forward(self, x):
        ...
        # calculate attention
        attn_scores = torch.bmm(q, k)  # (N, T, (H*W))
        attn_scores = attn_scores / (E ** 0.5)
        attn_scores = torch.softmax(attn_scores, dim=-1)

        v = v.permute(0, 2, 1)  # (N, (H*W), E)
